...
def interpdata(data, oldtime, newtime):
    ...
    if not len(oldtime) == data.shape[0]:
        raise IndexError(
            "oldtime must have same number of elements as data has rows.")
...
def sincinterp1D(data, oldtime, newtime, cutoff_mult=1.0, window=1):
    ...
    print("Doing sinc interpolation with cutoff=%0.3f and %d lobes." %
          (cutoff, window))
    ...
    for ndi in range(len(newtime)):
        for di in range(len(oldtime)):
            newdata[ndi] += sincfun(cutoff, newtime[ndi] -
                                    oldtime[di], window) * data[di]
    return newdata
...
def kernel_density_interp2D(data, oldtime, newtime, bandwidth=1.0):
    """Interpolates the columns of `data`, assuming that the i'th row of data corresponds to
    oldtime(i). A new matrix with the same number of columns and a number of rows given
    by the length of `newtime` is returned using kernel density weighted mean.

    `bandwidth` controls the smoothing of the interpolation.
    """

    def _gaussian_kernel(distance, bandwidth):
        """Computes the Gaussian kernel values for a given distance and bandwidth."""
        return np.exp(-0.5 * (distance / bandwidth) ** 2) / (bandwidth * np.sqrt(2 * np.pi))

    newdata = np.zeros((len(newtime), data.shape[1]))

    for i, new_t in enumerate(newtime):
        # Compute distances from this new time to all old times
        distances = new_t - oldtime

        # Apply the Gaussian kernel to these distances to compute weights
        weights = _gaussian_kernel(distances, bandwidth)

        # Make sure weights sum to 1 to preserve signal magnitude
        weights /= weights.sum()

        # Compute the weighted mean for each column
        for col in range(data.shape[1]):
            newdata[i, col] = np.dot(weights, data[:, col])

    return newdata


def nearest_neighbor_interp2D(data, oldtime, newtime):
    """
    Interpolates the columns of `data`, assuming that the i'th row of data corresponds to
    oldtime(i). A new matrix with the same number of columns and a number of rows given
    by the length of `newtime` is returned. This function performs nearest neighbor
    interpolation.

    Each value in `newtime` is matched to the nearest temporal neighbor in `oldtime`, and
    the corresponding values from `data` are used to construct the new dataset.
    """
    newdata = np.zeros((len(newtime), data.shape[1]))

    # Iterate over each new time point
    for i, new_t in enumerate(newtime):
        # Find the index of the closest old time point
        idx = np.abs(oldtime - new_t).argmin()
        # print('replacing', new_t, 'with', oldtime[idx])

        # Use the data from the closest old time point
        newdata[i, :] = data[idx, :]
