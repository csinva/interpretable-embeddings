...
def greedy_search(queries_embeddings, queries_data_train, tsv_file, vectorizer, corpus_vectors, base_score):
    ...
    while len(candidate_indices) > 0:
        for i in candidate_indices:
            ...
            correct_matches_with_embeddings, query_results_with_embeddings = process_queries(queries, queries_data_train, X, tsv_file, vectorizer, lr=lr, append_coeff=append_coeff, indices=indices_append, embeddings=queries_embeddings_train, use_only_query_embeddings=False)
...
if __name__ == "__main__":
    ...
    X = corpus_vectors
    correct_matches_without_embeddings, query_results_without_embeddings = process_queries(queries, queries_data, X, tsv_file, vectorizer, use_only_query_embeddings=False)
    ...
    greedy_embeddings, base_score = greedy_search(queries_embeddings, queries_data_train, tsv_file, vectorizer, corpus_vectors, base_score)
