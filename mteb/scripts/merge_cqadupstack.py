...
if len(files) == len(TASK_LIST_CQA):
    ...
    # Move TASK_LIST_CQA files to a separate folder
    processed_folder = os.path.join(results_folder, "processed")
    if not os.path.exists(processed_folder):
        os.makedirs(processed_folder)
    for task in TASK_LIST_CQA:
        src_file_path = os.path.join(results_folder, f"{task}.json")
        dst_file_path = os.path.join(processed_folder, f"{task}.json")
        shutil.move(src_file_path, dst_file_path)
    logger.info(f"Moved TASK_LIST_CQA files to {processed_folder}")
else:
    logger.warning(
